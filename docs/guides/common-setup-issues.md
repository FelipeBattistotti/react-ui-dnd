...
For detectable setup issues we try to log information in the `console` for `development` builds of `react-ui-dnd`. If things are not working, first thing to do is check your `console`.
...
We go over this in our [Get started with `react-ui-dnd` course](https://egghead.io/lessons/react-move-items-between-columns-with-react-ui-dnd-using-ondragend)
